...
Before EFA, the script now checks variable suitability using the KMO and Bartlett tests. Mixed-type correlations are computed with `psych::mixedCor` (passing the continuous, ordinal and binary column indices explicitly) and then smoothed with `psych::cor.smooth` so the matrix is positive definite.
## Environment Setup
